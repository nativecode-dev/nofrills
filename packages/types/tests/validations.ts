...
describe('when using types library for validation', () => {
  const validate: (value: any, type: string) => boolean = types.Types.validate
  const testtype: Partial<types.Type> = {
    properties: {
      max: 5,
      min: 2,
      required: true,
  ...
  describe('to validate primitive values', () => {
    ...
    it('should validate value is an array', () => {
      expect(validate([1, 2, 3], 'array')).to.equal(true)
    })
    ...
    it('should validate value is a date', () => {
      expect(validate(new Date(), 'date')).to.equal(true)
    })
    ...
    it('should validate value is an error', () => {
      expect(validate(new Error(), 'error')).to.equal(true)
    })
    ...
    it('should validate value is a number', () => {
      expect(validate(123, 'number')).to.equal(true)
    })
    ...
    it('should validate value is an object', () => {
      expect(validate({}, 'object')).to.equal(true)
    })
    ...
    it('should validate value is a string', () => {
      expect(validate('string', 'string')).to.equal(true)
    })
    ...
    it('should validate value is a timestamp', () => {
      expect(validate(Date.now(), 'timestamp')).to.equal(true)
    })
  ...
  describe('to validate custom type values', () => {
    it('should validate value is an email', () => {
      expect(validate('nobody@nowhere.com', 'email')).to.equal(true)
    })
    ...
    it('should validate value is a postal code', () => {
      expect(validate('12345-1234', 'postalcode')).to.equal(true)
    })
    ...
    it('should validate string value length is required', () => {
      expect(validate(null, 'test')).to.equal(false)
    })
  })
